...
describe '投稿テスト' do
  let!(:list) { create(:list,title:'hooge',body:'body') }
  describe 'トップ画面(top_path)のテスト' do
    before do
      visit top_path
    end
    context '表示の確認' do
      it 'トップ画面(top_path)に「ここはTopぺーじです」が表示されるか' do
  ...
  describe '投稿画面のテスト' do
    ...
    context '表示の確認' do
      it 'new_list_pathが"/lists/new"であるか' do
        expect(content_path).to eq('/lists/new')
      end
    ...
    context '投稿処理のテスト' do
      it '投稿後のリダイレクト先は正しいか' do
        ...
        fill_in 'list[body]', with: Faker::Lorem.charecters(number:30)
  ...
  describe '一覧画面のテスト' do
    ...
    context '一覧の表示とリンクの確認' do
      it '一覧表示画面に投稿されたものが表示されているか' do
        expect(pege).to have_content list.title
  ...
  describe '詳細画面のテスト' do
    ...
    context 'リンクの遷移先の確認' do
      it '編集の遷移先は編集画面か' do
        ...
        edit_link.click
        expect(page).to eq('lists/' + list.id.to_s + '/edit')
      end
    end
